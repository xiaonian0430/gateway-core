...
class Core
{
    ...
    protected $runMode = 'produce';

    function __construct()
    {
        defined('SERVER_ROOT') or define('SERVER_ROOT', realpath(getcwd()));
        $eventFile = SERVER_ROOT.'/event.php';
        if(!file_exists($eventFile)){
            die(Color::red("event.php file miss ,check again or run again \n"));
        }else{
    ...
    function createServer()
    {
        ...
        $ret = GlobalEvent::mainServerCreate(ServerManager::getInstance()->getEventRegister());
    ...
    private function sysDirectoryInit(): void
    {
        ...
        defined('SERVER_TEMP_DIR') or define('SERVER_TEMP_DIR', $tempDir);

        $logDir = Config::getInstance()->getConf('LOG.dir');
        if (empty($logDir)) {
            $logDir = SERVER_ROOT . '/Log';
        ...
        defined('SERVER_LOG_DIR') or define('SERVER_LOG_DIR', $logDir);
    ...
    private function registerErrorHandler()
    {
        ...
        if (!$logger instanceof LoggerInterface) {
            $logger = new DefaultLogger(SERVER_LOG_DIR);
        }
    ...
    private function registerDefaultCallBack(Server $server, int $serverType)
    {
         ...
         * 注册默认回调
         */
        if (in_array($serverType, [2, 3], true)) {
    ...
    public function loadEnv()
    {
        ...
        $file = SERVER_ROOT . "/config/{$this->runMode}.php";
        if (!file_exists($file)) {
    ...
    private function extraHandler()
    {
        ...
        $config->setTempDir(SERVER_TEMP_DIR);
